...
def main():
    ...
    # load the fact table
    load_lineitems(cur, order_customer_map)
...
def load_partsupps(cursor):
    ...
    (ps_partkey integer, ps_suppkey integer, ps_availqty integer, ps_supplycost real, ps_comment text,
     PRIMARY KEY (ps_partkey, ps_suppkey),
     FOREIGN KEY (ps_suppkey) REFERENCES suppliers(s_suppkey),
     FOREIGN KEY (ps_partkey) REFERENCES parts(ps_partkey))
    '''
    cursor.execute(query)
...
# main fact table
def load_lineitems(cursor, order_customer_map):
    query = '''CREATE TABLE lineitems
    (l_revenue real, l_custkey integer, l_orderkey integer, l_partkey integer, l_suppkey integer,
     l_linenumber integer, l_quantity integer,
     ...
     FOREIGN KEY(l_partkey, l_suppkey) REFERENCES partsupps(ps_partkey, ps_suppkey))
    '''
    cursor.execute(query)
